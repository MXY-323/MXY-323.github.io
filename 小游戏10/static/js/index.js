...
var gameMonitor = {
	...
	getScore : function(){
		...
		var user = 1;
		if(score==0){
			$('#scorecontent').html('真遗憾，您竟然<span class="lighttext">一张</span>高铁票都没有抢到！');
		...
		$('#scorecontent').html('您在<span id="stime" class="lighttext">2378</span>秒内抢到了<span id="sscore" class="lighttext">21341</span>张高铁票<br>超过了<span id="suser" class="lighttext">31%</span>的用户！');
		$('#stime').text(time);
		$('#sscore').text(score);
		$('#suser').text(user+'%');
		$('.btn1').text('请小伙伴抢车票').removeClass('playagain').addClass('share');
		$(".txt").removeClass('hide').addClass('show');
	},
	isMobile : function(){
